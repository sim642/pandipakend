    ...
    def scrape(self, term: str = ""):
        ...
        def scrape(term, fwd, depth=0):
            ...
            if fwd:
                ...
                for j in range(1, len(term)): # TODO: can check better with suffix_tree (matching statistics?)
                    # for j in range(i, len(term)):
                    i = 0 # no point in checking others, they would've been rejected at upper scrape depths
                    if term[i:j] in self.term_set:
                        logger.debug("skipping %s %s", term, term[i:j])
                        return
